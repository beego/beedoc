...
反之，如果只使用 Raw 查询和 map struct，是无需这一步的。您可以去查看 [Raw SQL 查询](/docs/mvc_model_rawsql)
...
详细的 struct 定义请查看文档 [模型定义](/docs/mvc_model_models)
...
但是在使用事务的时候，我们会返回`TxOrm`的实例。请参考[orm 事务](/content/mvc_model_transaction.md)
...
传入表名，或者 Model 对象，返回一个 [QuerySeter](/docs/mvc_model_query)
...
Raw 函数，返回一个 [RawSeter](/docs/mvc_model_rawsql) 用以对设置的 sql 语句和参数进行操作
