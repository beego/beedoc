...
Автоматическая документация это очень крутая возможность которую мы хотели иметь. Сейас это стало реальностью с Beego. Как я сказал, Beego не только ускоряет разработку API, но также делает API легко используемым для пользовател. Окей, давайте попробуем создать документацию, для этого вам надо просто создать новое API приложение с помощью команды `bee api beeapi`.
...
Комментарий который мы добавили устанавливает глобальную информацию. Вот список доступных настроек:
...
Документация вашего API готова. Откройте браузер и проверьте это.
...
2. Другие проблемы. Это используется в проектах разработчиков BeeGo, но если вы будете иметь неописанные проблемы, пожалуйста создайте issues для нас.
